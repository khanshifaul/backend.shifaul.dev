    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        const result = await this.prisma.$transaction(async (tx) => {
          ...
          // Atomically delete all refresh tokens for this session
          const deletedTokensCount = await tx.refreshToken.deleteMany({
            where: {
          ...
          this.logger.log(
            `✅ Atomic session invalidation: ${sessionId} (deleted ${deletedTokensCount.count} tokens)`
          );

          return {
            sessionId: session.sessionId,
            deletedTokens: deletedTokensCount.count,
          };
        });
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        const result = await this.prisma.$transaction(async (tx) => {
          ...
          // Atomically delete refresh tokens for all sessions
          const deletedTokensResult = await tx.refreshToken.deleteMany({
            where: {
              sessionId: { in: sessionIds_list },
              isActive: true,
            },
          });
          ...
          this.logger.log(
            `✅ Atomic batch invalidation: ${sessions.length} sessions, ${deletedTokensResult.count} tokens`
          );
  ...
  async cleanupExpiredSessionsAtomic(): Promise<TransactionResult<{
    ...
    try {
      const result = await this.prisma.$transaction(async (tx) => {
        ...
        const tokensDeletedResult = await tx.refreshToken.deleteMany({
          where: {
            sessionId: { in: sessionIds_list },
          },
        });
        ...
        this.logger.log(
          `🧹 Cleanup: ${sessionsDeletedResult.count} sessions, ${tokensDeletedResult.count} tokens`
        );

        return {
          sessionsDeleted: sessionsDeletedResult.count,
          tokensDeleted: tokensDeletedResult.count,
        };
      });
